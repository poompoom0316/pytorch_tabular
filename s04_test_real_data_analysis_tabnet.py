...
from pytorch_tabular.models import TabNetModelConfig, TabNetModel
...
def main():
    ...
    for i, (train_idx, test_idx) in enumerate(cv.split(X=data, y=data.target.values)):
        ...
        # Training the Model
        tabular_mode.fit(train=train, validation=val, max_epochs=5, loss=weighted_loss)
    ...
    sample_submisson.to_csv("Analysis/submission_1.csv", index=False)
