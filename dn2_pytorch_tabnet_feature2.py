...
def data_load():
    ...
    cat_col_names_pre = list(data.filter(like="cat").columns)
    num_col_names = list(data.filter(like="cont").columns)

    for col in cat_col_names_pre:
    ...
    #     print(f"TIME {time.time()-t1}: {num_col}")
    #     test_data[num_col] = np.clip(data[num_col], data[num_col].min(), data[num_col].max())
    #
    ...
    #     test_data[f'q_{num_col}'] = pd.cut(test_data[num_col], bins=bins_, labels=False, include_lowest=True).astype(str)
...
def main():
    ...
    sample_submisson.to_csv("Analysis/submission_3_node.csv", index=False)
